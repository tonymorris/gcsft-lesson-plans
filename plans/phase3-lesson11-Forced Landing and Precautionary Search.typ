...
,aim: [
Conduct a simulated forced landing into a suitable field and conduct all emergency procedures (ABC checks). Conduct a precautionary search and landing.
]
...
S standard must be demonstrated on two separate flights before area solo is authorised. The area solo suitability assessment (L4.6) may count as the second S standard flight.
]
...
,theory_knowledge: [
- Human Factors — mind-set, false hypothesis in emergency *(Pg 34–35)*
...
Equipment required: Topographic/VNC chart of training area, whiteboard \
...
- When it applies: deteriorating weather, developing mechanical problem, fuel concern
- Decision to search: made before conditions become critical — not a last resort
- The sequence for the precautionary search and landing should be as follows:
- Join "circuit" around field at 1000 ft AGL
- First pass at ~1000 ft AGL: assess surface, slope, obstacles, continue circuit until downwind
- Configure for normal approach, turn base and continue approach to 500 ft AGL, level off
- Second pass at ~500 ft AGL: confirm surface; decide to land or reject
- Remain at 500 ft AGL, continue in a low level circuit until final approach, descend to 300 ft AGL, level off
- Third pass at ~300 ft AGL: if any doubt about the surface
- Go around to 1000 ft AGL, confirm intentions and position with ATC (PAN-PAN)
...
- Receives brief; specifies training go-around height before flight
- Assumes instructor role to deliver ABCDEF ground brief content above
]

,in_flight_tasks:[
*Note: S standard must be demonstrated on two separate flights before area solo authorisation. The area solo suitability assessment (L4.6) may count as the second.*

#text(size:12pt)[*Observer/PIC Phase — Departure and Transit*]
Student operates as PIC. Instructor is an observer. Student conducts all ground operations, takeoff, departure, and transit. Student actively scans for and notes suitable landing fields during transit — this is now an ongoing habit.
...
- *Altitude loss in slow flight during PSL*: Power + Attitude = Performance
]
...
*Note: S standard must be demonstrated on two separate flights. The area solo suitability assessment (L4.6) may count as the second.*

#text(size:12pt)[*Progressing (P) — may advance to L4.5*]
...
- *FLWOP circuit*: 1000 ft at low-key; height managed well, such that the field is made with sufficient stopping distance; sideslip if required
- *Precautionary search*: Two passes at correct heights; clear landing/reject decision with correct reasoning
...
,safety:[
- *Weather*: Ceiling >2500 ft AGL; wind \<25 knots
...
,homework:[
- *Memorise the ABC checks completely before the next lesson:*
  - A — Attitude (glide attitude immediately)
  - B — Best field (6-S: Size, Surface, Slope, Situation, Surroundings, Services; aim point at one-third)
  - C — Checks (FAST: Fuel, Air, Switches, Throttle)
  - D — Declare (MAYDAY: callsign, position, problem, plan, POB)
  - E — Explain (passenger brief: seatbelt, door unlatched, brace)
  - F — Final (all switches off; flap before master if electric flaps fitted)
- Read Bob Tait's RAAus Study Guide:
  - Human Factors: Pg 34–35, 43–51 (mind-set, false hypothesis, TEM, undesired aircraft states)
  - VHF Radio: Pg 62–63 (MAYDAY call format)
  - Air Legislation: Pg 88 (pilot emergency authority)
- Access the ERSA for Southport Airport (YSPT) and extract the elevation, CTAF, circuit direction and runway. Is there anything non-standard about Southport?
- Prepare for E16 (Unusual Attitudes and Emergencies) next lesson:
  - Aerodynamics: Pg 106, 165, 170–173 (spiral dive, load factor, spins, dynamic stall)
  - Human Factors: Pg 20, 44, 48, 51 (disorientation, undesired aircraft states, countermeasures)
- Reflect:
  - "At what point in the FLWOP circuit would you reassess your field choice? What would cause you to change it?"
  - "You change the fuel tank, and 30 seconds later the engine stops. What is your first action? Start conducting the ABC checks or change the tank back?"
