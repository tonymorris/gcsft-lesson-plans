...
,overview: [
Lesson 4.3 pairs with E7 (Phase 1, Lesson 6) to satisfy the two-flight requirement for stalling competency. Where E7 focused on recognising and recovering from the approaching stall in straightforward configurations, E10 places stalling in realistic flight scenarios — the go-around, turns, and emergency operations including EFATO. The student must connect the aerodynamic knowledge from E7 and E9 to the actual high-risk scenarios. Spin entry awareness is discussed and demonstrated as a visual familiarisation exercise — intentional spinning is not conducted within this syllabus, and pro-spin inputs below manoeuvring speed are not permitted.

The student operates as PIC for all trained flight portions. The instructor assumes the instructor role for demonstration only.
]
...
- [HF] Manage startle response in unexpected stall entries; brief management strategies before flight; control handover procedure confirmed
- [CRM] Student briefs each scenario to the instructor before entry; instructor confirms understanding
]
...
- Receives brief; confirms smooth air and height requirements are met
- Assumes instructor role for ground brief of new content only
]
...
Student conducts return transit, circuit join, radio calls, and landing independently. Instructor may assist with inbound radio calls during transit; hands back at circuit entry.
]
...
,post_flight:[
- *Student Leads Debrief* (10 min): Student reviews each scenario; identifies the TEM chain; self-assesses recovery technique and height loss.
- *Instructor Supplements* (5 min): Adds observations; reinforces the connection between scenario context and stall risk.
- *Theory Check* (5 min): Student explains why aileron worsens a wing drop and what prevents spin entry.
- *Next Steps*: Assign homework. Note: with E10 complete and E7 previously recorded, the two-flight stalling requirement is now satisfied.
]

,standards_for_progression:[
#text(size:12pt)[*Progressing (P) — may advance to L4.4*]
- *Scenario stalls*: Identifies pre-stall symptoms before full stall in at least two scenarios; recovery correct with minor prompting
- *Turning stall*: Rudder applied on wing drop with one prompt; ailerons neutral; height loss ≤400 ft
- *Go-around stall prevention*: Identifies and prevents stall entry with instructor guidance
- *EFATO scenario*: Maintains coordination and glide attitude with minor prompting
...
- *Scenario stalls (all configurations)*: Consistently identifies pre-stall symptoms; recovery at stall onset; height loss ≤300 ft; no overcontrolling
- *Turning stall*: Relax backpressure; rudder promptly on wing drop; ailerons neutral; recovery smooth; height loss ≤300 ft
- *Go-around stall prevention*: Consistently prevents stall entry; controlled pitch attitude; no instructor intervention
- *EFATO scenario*: Correct glide attitude; coordinated; no excess backpressure
- *HASSELL*: Completed before sequence and every 15 minutes without prompting
- *Student-as-PIC*: All trained portions conducted without instructor input

- *Outcome*: Recorded in training record as P or C. Note: E7/E10 two-flight requirement now satisfied.
]

,safety:[
- *Weather*: Horizon clearly visible; ceiling >3500 ft AGL; smooth air essential; wind \<25 knots. Postpone if turbulent.
- *Altitude*: All stall exercises above 2500 ft AGL; all recoveries complete by 2500 ft AGL; HASSELL minimum height enforced
- *Emergency Procedures*: Forced landing area pre-selected before HASSELL; instructor ready to intervene if spin entry appears imminent or minimum height is approached
- *TEM*:
...
- Prepare for E14 (Forced Landing and Precautionary Search) next lesson:
  - Human Factors: Pg 34–35 (mind-set, false hypothesis in emergency decision-making)
  - VHF Radio: Pg 62–63 (MAYDAY and PAN calls)
- Reflect:
  - "Name three realistic circuit scenarios where an inadvertent stall could develop. For each: what is the TEM chain? What is the prevention?"
  - "What is the one thing that separates a stall with a wing drop from a spin, and what does this mean for your recovery technique?"
]
)
