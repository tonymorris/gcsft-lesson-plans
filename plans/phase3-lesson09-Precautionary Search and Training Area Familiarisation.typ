...
Two S standard flights on E14a are required before area solo authorisation. This lesson is the first S standard flight; the area solo suitability assessment (L12) is the second.
]

,learning_outcomes: [
#text(size:12pt)[*Knowledge*]
- Apply the ADM framework to the precautionary search decision: Time, Assess, Predict, Evaluate
- Explain when a precautionary search is appropriate: deteriorating weather; developing mechanical concern; fuel uncertainty; any situation where continuing to destination is uncertain
- Describe why the precautionary search decision must be made proactively — not reactively
- Describe the precautionary search procedure: inspection pass heights; what to assess on each pass; the landing commitment decision; when a third pass is required
- Explain the PAN-PAN call: when to transmit; format; difference from MAYDAY
- Locate and interpret the ERSA entry for Southport Airport (YSPT): elevation, CTAF, circuit direction, runway details, any non-standard items

#text(size:12pt)[*Skills*]
- Execute E9 (Advanced Turning) to S standard without instructor input: HASSELL, steep turns, spiral dive check
- Fly the precautionary search sequence: join to field; first pass at 1000 ft AGL; circuit to downwind; approach to 500 ft AGL; second pass; circuit; approach to 300 ft AGL (aerodrome only) if required; landing/go-around decision
- Transmit a simulated PAN-PAN call with correct format: callsign, position, problem, plan
- Interpret an ERSA entry and extract operationally relevant information
...
,theory_knowledge: [
- Human Factors — Aviation Decision Making, TEM *(Pg 43–51)*
- Human Factors — mind-set, false hypothesis *(Pg 34–35)*
- VHF Radio — PAN-PAN call format *(Pg 62–63)*
- Air Legislation — ERSA interpretation, alternate aerodromes *(Pg 86, 90)*
]
...
- The student will conduct HASSELL, brief the E9 sequence, and execute without instructor input
- If E9 is not at S standard: the instructor will note this in the training record; the PSL flight will continue for exposure but this will be recorded separately; a remediation flight is scheduled before the area solo gate
- Instructor decision rule: if the E9 check clearly requires more than 15 minutes to address, note NC and proceed to PSL; do not sacrifice the PSL content to fix E9 in this lesson
...
- Receives full brief; queries as needed to confirm understanding of PSL procedure and ERSA items
- Confirms conditions are suitable; confirms HASSELL minimum height for E9
- Delivers the ADM and PSL ground brief content above
- Does not provide any content on the E9 check — this is student-led
]

,in_flight_tasks:[
#text(size:12pt)[*Student-as-PIC — All Trained Portions*]
Student handles all ground operations, startup, taxi, runup, radio calls, and takeoff without prompting. Instructor is a silent observer throughout trained portions.

#text(size:12pt)[*E9 Check — Student Led, Instructor as Silent Observer*]
The student completes and executes the E9 check entirely without instructor input:
- Student completes HASSELL check; instructor is silent
- Student executes steep turns at 45° and 60° AoB (left and right); instructor observes and records silently
...
,post_flight:[
- *Student Leads Debrief* (10 min): Student reviews the E9 check against S standard; reviews the PSL attempts; applies ADM review to each decision point.
- *Instructor Supplements* (10 min):
  - E9 result stated clearly: S or NC with specific evidence
  - If NC: explain exactly what was not at S standard; confirm a remediation flight is needed before the area solo gate; PSL result is independent and unaffected
  - PSL result: first S standard flight recorded if S standard was met; specific comments on pass heights, decision quality, and ADM application
  - ERSA debrief: were the YSPT details correct?
- *Scenario Reflection* (5 min): "You are 8 NM from home at 1500 ft, and the ceiling is at 1800 ft and dropping. Walk me through your ADM process from now."
]

,standards_for_progression:[
#text(size:12pt)[*E9 Check — Recorded separately*]
- *S standard confirmed*: Steep turns ±5° bank, ±150 ft altitude; spiral dive recovery correct; HASSELL completed without prompting — recorded as S confirmed in Phase 3
- *NC*: Specific deficiency noted; remediation flight scheduled before area solo gate; does not affect E14a result

#text(size:12pt)[*E14a (Precautionary Search) — Two flights at S required; this lesson is the first*]

#text(size:12pt)[*Progressing (P) — may advance to Lesson 10*]
- *PSL*: Two inspection passes completed at approximately correct heights; landing/reject decision made with guidance; PAN-PAN attempted
- *ADM*: Framework applied with instructor prompting; Time question asked with guidance
...
- *TEM*:
  - E9 check: instructor is observer but maintains safety readiness; take control if spiral dive recovery is incorrect or minimum height is threatened
  - PSL at 300 ft AGL: only at the home aerodrome; never over a field; brief this restriction before flight
  - Monitor for spatial disorientation during low, slow PSL passes
  - Maintain SA regarding other training area traffic throughout
]

,homework:[
- Read Bob Tait's RAAus Study Guide:
  - Human Factors: Pg 34–35, 43–51 (mind-set, false hypothesis, TEM, ADM)
  - VHF Radio: Pg 62–63 (PAN-PAN format)
  - Air Legislation: Pg 86, 90 (alternate aerodrome, emergency pilot authority)
- Access the ERSA for Southport Airport (YSPT); write out: elevation, CTAF, circuit direction, runway details, non-standard items
- Prepare for Lesson 10 (Forced Landing Without Power):
